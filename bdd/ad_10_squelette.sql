...
COMMENT ON COLUMN r_adresse.an_adresse.id_ban_adresse IS 'Identifiant d''adresse de la BAN au format UUID V4 (cet identifiant doit être commun à toutes les positions d''une même adresse => pas géré ici car le trigger empêche la saisie de plusieurs même adresses pour le moment)';
